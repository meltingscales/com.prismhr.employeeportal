...
final class RunSuspend implements Continuation<Unit> {
    ...
    public void resumeWith(Object obj) {
        synchronized (this) {
            setResult(Result.m65boximpl(obj));
            notifyAll();
    ...
    public final void await() {
        synchronized (this) {
            while (true) {
                Result<Unit> m1253getResultxLWZpok = m1253getResultxLWZpok();
                if (m1253getResultxLWZpok == null) {
                    wait();
                } else {
                    ResultKt.throwOnFailure(m1253getResultxLWZpok.m75unboximpl());
                }
            }
        }
    }
