...
public final class DispatchedTaskKt {
    ...
    public static final <T> void dispatch(DispatchedTask<? super T> dispatchedTask, int i) {
        ...
        if (isDispatchedMode(i) && (delegate$kotlinx_coroutines_core instanceof DispatchedContinuation) && isCancellableMode(i) == isCancellableMode(dispatchedTask.resumeMode)) {
            ...
            if (coroutineDispatcher.isDispatchNeeded(context)) {
                coroutineDispatcher.mo1523dispatch(context, dispatchedTask);
                return;
            } else {
    ...
    public static final <T> void resume(DispatchedTask<? super T> dispatchedTask, Continuation<? super T> continuation, int i) {
        ...
        Object m66constructorimpl = Result.m66constructorimpl(successfulResult$kotlinx_coroutines_core);
        if (i == 0) {
            continuation.resumeWith(m66constructorimpl);
        } else if (i == 1) {
            DispatchedContinuationKt.resumeCancellableWith(continuation, m66constructorimpl);
        } else if (i != 2) {
            ...
            if (continuation != null) {
                ...
                try {
                    dispatchedContinuation.continuation.resumeWith(m66constructorimpl);
